...
class DNA_ALIGN(ALIGN):
	def __init__(self, args, fq1, fq2):
		"""
		DNA_ALIGN constructor
		"""
		...
		if self.args.mapper is None:
			# default mapper for dna is bwamem
			self.args.mapper = "bwamem"		

	def do_align(self):
		"""
		DNA_ALIGN.do_align: for each command in CMD_LIST, replace with sample name and bam output directory, as well as fq1 and fq2"
		"""
		my_cmd = ""
		...
		for m in cmd_order_list:
			...
			exec(replace_fastq1)
			exec(replace_fastq2)
			logging.debug("[DNA_ALIGN] after replacement: " + my_cmd)
			yield my_cmd
		
		if self.args.dedup:
			cmd_order_list = ["dedup", "mvbam", "indexbam"]
			for m in cmd_order_list:
				replace_mapped = "my_cmd = re.sub(\"MAPPED\", \"%s\", CMD_LIST.%s)" % (self.mapped_dir, m)
				replace_sample_name = "my_cmd = re.sub(\"SAMPLE_NAME\", \"%s\", my_cmd)" % (self.sample_name)
				exec(replace_mapped)
